// 리터니즈 워크시트 완료 건 → 리터니즈_입고_히스토리 이관 API
//
// archive-completed.js(판토스_입고리스트 → 판토스_입고_히스토리)와 판정 규칙은
// 동일하다. 다만 리터니즈 시트는 데이터가 2행부터 시작하고(판토스는 3행부터),
// 열 구성도 다르므로 별도 엔드포인트로 분리했다.
//
// 판정 규칙:
//   - 같은 주문번호를 그룹으로 묶는다.
...
// 열 범위도 하드코딩하지 않고 헤더 행 길이로 매번 계산한다 — 리터니즈 시트에
// "메모" 열이 추가된 사례처럼 앞으로 열이 늘거나 밀려도 안전하게 하기 위함.
// 행을 그대로 복사하는 방식이라 리터니즈 시트와 히스토리시트의 열 구성이
// 완전히 같아야 하므로, 이관 전에 헤더가 일치하는지 검증하고 다르면 중단한다.
//
...
function findArchiveRows(rows, orderNoIdx, doneDateIdx) {
  ...
  rows.forEach((row, i) => {
    ...
    groups[orderNo].push({ absRow: DATA_START_ROW + i, values: row });
  });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error('[archive-returnize]', err);
