...
//
// 로직: 반품입고시트 T열(입고코드번호 = 주문번호+상품명옵션)을 정규화한 뒤,
// [자사몰]/[외부몰]/[불량] 교환 접수시트의 P열("[자동]" 식별자, 같은 방식으로
// 정규화됨)과 매칭해서 어느 교환 시트에서 왔는지로 구분값을 매긴다.
// 못 찾으면 "반품"으로 처리.
//
// 사용법:
//   node scripts/recompute-category.mjs           # 실제로 E열에 씀
...
async function buildCategoryMap(jwt) {
  ...
  for (const src of CATEGORY_SOURCES) {
    const pCol = await sheetsGet(jwt, EXCHANGE_SS_ID, `${src.sheet}!P2:P`);
    pCol.forEach(row => {
...
async function main() {
  ...
  console.log('2) 반품입고시트 T열(입고코드번호) 읽는 중...');
  const tCol = await sheetsGet(jwt, RETURNS_SS_ID, `'${PENDING_SHEET}'!T3:T`);
  console.log('  ', tCol.length, '행');

  console.log('3) 구분값 계산 중...');
  ...
  console.log('4) E열에 쓰는 중...');
  const range = `'${PENDING_SHEET}'!E3:E${2 + newE.length}`;
