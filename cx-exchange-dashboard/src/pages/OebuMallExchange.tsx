...
export const OebuMallExchange: React.FC = () => {
  ...
  const { channelStats, trendData, stats, optionAnalyses, productList } = useMemo(() => {
    if (!exchangeData?.data.oebuMall) return { channelStats: [], trendData: [], stats: { total:0, mom:15, defectiveRate:0 }, optionAnalyses: [], productList: [] };
    ...
    const bulryangIds = new Set((exchangeData.data.bulryang || []).map(r => r['주문번호']));
    ...
    const statsArr = Object.entries(groups).map(([name, count]) => ({ name, count })).sort((a,b)=>b.count-a.count);
    ...
    return { 
      channelStats: statsArr, trendData: trendArr, stats: { total: oebu.length, mom: 15, defectiveRate: oebu.length > 0 ? Math.round((defectiveCount/oebu.length)*100) : 0 },
    ...
    <div className="space-y-8 max-w-[1600px] mx-auto w-full pb-20 px-4 lg:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         ...
         <div className="bg-slate-900 p-8 rounded-3xl shadow-xl flex items-center justify-between text-white">
            <div><p className="text-[10px] font-black opacity-50 uppercase mb-1">전기 대비 증감</p><h3 className="text-4xl font-black text-orange-400">+{stats.mom}%</h3></div>
            <Icon name="trending_up" className="text-3xl opacity-20" />
         </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
         <div className="lg:col-span-4 bg-white p-8 rounded-[32px] border border-slate-100 shadow-sm"><h4 className="font-black text-slate-900 text-lg mb-8 uppercase text-center">채널별 교환 점유율</h4><div className="h-[280px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={channelStats.slice(0, 3)} innerRadius={65} outerRadius={90} paddingAngle={8} dataKey="count">{channelStats.map((_e, i) => <Cell key={i} fill={['#f97316', '#fb923c', '#fdba74', '#fed7aa', '#ffedd5'][i % 5]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer></div></div>
