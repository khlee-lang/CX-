...
const RATE_BAND_HEX: Record<string, string> = {
  emerald: '#10b981',
  amber: '#f59e0b',
  orange: '#f97316',
  rose: '#ef4444',
...
export const OebuMallExchange: React.FC = () => {
  ...
  const { channelStats, trendData, stats, optionAnalyses, productList, oebuRows } = useMemo(() => {
    if (!exchangeData?.data.oebuMall) return { channelStats: [] as ChannelStat[], trendData: [], stats: { total:0, mom:0, defectiveRate:0 }, optionAnalyses: [], productList: [], oebuRows: [] as any[] };
    ...
    const statsArr: ChannelStat[] = Object.entries(groups).map(([name, g]) => {
      const shipped = shipIdx ? lookupChannel(shipIdx, name) : null;
      const r = computeRate(g.count, shipped, 0);
      return {
        name,
        count: g.count,
  ...
  }, [exchangeData, startDate, endDate, selectedProduct, shipIdx]);

  const { overallRate, matchCoverage, jasaScopedCount } = useMemo(() => {
    if (!shipIdx) return { overallRate: null, matchCoverage: null, jasaScopedCount: 0 };
    // 네이버페이는 자사몰(카페24) 주문이라 출고량이 외부몰 분모에 없다 → 분자에서도 제외
    const scoped = oebuRows.filter((r) => isJasaScopedChannel(r['채널명'])).length;
    const r = computeRate(stats.total - scoped, shipIdx.total.oebu);
    const coverage = buildMatchCoverage(oebuRows, shipIdx);
    return { overallRate: r, matchCoverage: coverage, jasaScopedCount: scoped };
  }, [shipIdx, stats, oebuRows]);

  const channelColumns: Column<ChannelStat>[] = [
    {
      key: 'name', header: '채널',
      render: (ch) => <span className="font-black text-slate-800">{ch.name}</span>,
    },
    {
      key: 'count', header: '접수 건수', align: 'right',
      render: (ch) => <span className="font-black text-slate-900">{ch.count.toLocaleString()}</span>,
    },
    {
      key: 'share', header: '비중', align: 'right',
      render: (ch) => <span className="font-bold text-slate-500">{stats.total > 0 ? Math.round((ch.count / stats.total) * 100) : 0}%</span>,
    },
    {
      key: 'shipRate', header: '출고 처리율', align: 'right',
      render: (ch) => <span className={`font-black ${ch.shipRate >= 90 ? 'text-emerald-600' : 'text-rose-600'}`}>{ch.shipRate}%</span>,
    },
    {
      key: 'leadMedian', header: '리드타임 (중앙값)', align: 'right',
      render: (ch) => <span className="font-bold text-slate-700">{ch.leadMedian !== null ? `${ch.leadMedian}일` : '-'}</span>,
    },
    {
      key: 'defects', header: '불량 접수', align: 'right',
      render: (ch) => <span className={`font-black ${ch.defects > 0 ? 'text-rose-600' : 'text-slate-300'}`}>{ch.defects > 0 ? `${ch.defects}건` : '-'}</span>,
    },
    {
      key: 'shipped', header: '출고량', align: 'right',
      render: (ch) => (
        <span className="font-bold text-slate-500">
          {ch.shipped !== null
            ? ch.shipped.toLocaleString()
            : isJasaScopedChannel(ch.name) ? '자사몰 포함' : '-'}
        </span>
      ),
    },
    {
      key: 'rate', header: '교환율', align: 'right', sortValue: (ch) => ch.rate,
      render: (ch) =>
        ch.rate === null && isJasaScopedChannel(ch.name) ? (
          <span
            title="자사몰 비회원 주문이라 출고량이 카페24(신)에 포함되어 있어 채널 단위 교환율을 분리할 수 없습니다. 자사몰 교환율에 합산됩니다."
            className="inline-flex items-center rounded-lg font-black bg-indigo-50 text-indigo-500 px-2 py-0.5 text-[10px]"
          >
            자사몰 합산
          </span>
        ) : (
          <RateBadge rate={ch.rate} exchanges={ch.count} shipped={ch.shipped} />
        ),
    },
  ];

  // 채널별 교환율 비교 차트 데이터 — rate 있는 채널만, 내림차순
  const channelRateChart = channelStats
    .filter((ch) => ch.rate !== null)
    .map((ch) => ({ name: ch.name, rate: ch.rate as number }))
    .sort((a, b) => b.rate - a.rate);
  const avgOebuRate = channelRateChart.length > 0
    ? channelRateChart.reduce((s, c) => s + c.rate, 0) / channelRateChart.length
    : 0;
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto w-full pb-20 px-4 lg:px-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
         <div className="bg-orange-600 p-8 rounded-3xl shadow-xl flex items-center justify-between text-white">
            <div>
              <p className="text-[10px] font-black opacity-70 uppercase mb-1">외부몰 전체 교환율</p>
              <h3 className="text-4xl font-black">{overallRate?.rate !== null && overallRate?.rate !== undefined ? `${overallRate.rate.toFixed(1)}%` : '—'}</h3>
              <p className="text-[10px] font-bold opacity-70 mt-1">
                교환 {(stats.total - jasaScopedCount).toLocaleString()}건 / 출고{' '}
                {overallRate?.shipped != null ? `${overallRate.shipped.toLocaleString()}건` : '집계 중'}
              </p>
              {jasaScopedCount > 0 && (
                <p className="text-[10px] font-bold opacity-60 mt-1">
                  네이버페이 {jasaScopedCount}건 제외 (자사몰 출고분)
                </p>
              )}
            </div>
         </div>
      ...
      {channelRateChart.length > 0 && (
        <ChartCard
          title="채널별 교환율 비교"
          subtitle="rate 내림차순, 점선 = 외부몰 평균"
          height={Math.max(200, channelRateChart.length * 40)}
        >
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={channelRateChart} layout="vertical" margin={{ left: 24 }}>
              <CartesianGrid {...GRID_PROPS} horizontal={false} />
              <XAxis type="number" {...AXIS_PROPS} unit="%" />
              <YAxis type="category" dataKey="name" {...AXIS_PROPS} width={110} />
              <Tooltip formatter={(v) => [`${Number(v).toFixed(1)}%`, '교환율']} />
              <ReferenceLine x={avgOebuRate} stroke="#94a3b8" strokeDasharray="4 4" />
              <Bar dataKey="rate" radius={[0, 6, 6, 0]}>
                {channelRateChart.map((c, i) => (
                  <Cell key={i} fill={RATE_BAND_HEX[rateBand(c.rate, RATE_BANDS).color]} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
         <ChartCard className="lg:col-span-4" title="채널별 교환 점유율" height={280}>
            <ResponsiveContainer width="100%" height="100%">
               <PieChart>
                  <Pie data={channelStats.slice(0, 5)} innerRadius={65} outerRadius={90} paddingAngle={8} dataKey="count">
                     {channelStats.map((_e, i) => <Cell key={i} fill={['#f97316', '#fb923c', '#fdba74', '#fed7aa', '#ffedd5'][i % 5]} />)}
                  </Pie>
                  <Tooltip />
               </PieChart>
            </ResponsiveContainer>
         </ChartCard>
         <ChartCard className="lg:col-span-8" title="주간 교환 발생 추이" height={340}>
            <ResponsiveContainer width="100%" height="100%">
               <BarChart data={trendData}>
                  ...
                  <YAxis {...AXIS_PROPS} />
                  <Tooltip />
                  <Bar dataKey="simple" fill={SERIES_COLORS.shipped} stackId="a" />
                  <Bar dataKey="defective" fill={SERIES_COLORS.oebu} stackId="a" radius={[6, 6, 0, 0]} />
               </BarChart>
            </ResponsiveContainer>
         </ChartCard>
      </div>
      ...
      <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center">
         <div className="grid grid-cols-1 md:grid-cols-4 gap-12 w-full max-w-5xl">
            ...
            <div className="space-y-1"><h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">교환율 최고 채널</h5><p className="text-xl font-black text-orange-600">{channelRateChart[0]?.name || '-'}</p></div>
