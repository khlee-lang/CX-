...
const ReconcileSection: React.FC = () => {
  ...
  return (
    <section className="space-y-4">
      ...
      <p className="text-sm text-slate-500 -mt-2">반품입고시트의 미처리 {category} 건을 교환접수시트와 대조해 출고일·확인완료일을 자동 기입합니다.</p>
...
const RecomputeCategorySection: React.FC = () => {
  ...
  return (
    <section className="space-y-4">
      ...
      <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-4 flex gap-3 text-sm text-indigo-700 dark:text-indigo-300">
        <Icon name="info" className="mt-0.5 shrink-0" />
        <p>새 반품 건이 추가된 뒤 실행하면, 자사몰교환/외부몰교환/불량교환/반품 구분값을 다시 계산해서 채웁니다. (판토스_입고리스트 전용)</p>
      </div>
...
// ── 페이지 ─────────────────────────────────────────────────────
export const PantosOps: React.FC = () => {
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-black text-slate-800 dark:text-slate-100">판토스 반품 처리</h1>
        <p className="text-sm text-slate-500 mt-1">판토스_입고리스트 기준 반품-교환 연동, 구분값 재계산, 완료건 이관을 한 곳에서 처리합니다.</p>
      </div>

      <ReconcileSection />
      <div className="border-t border-slate-200 dark:border-slate-800" />
      <RecomputeCategorySection />
      <div className="border-t border-slate-200 dark:border-slate-800" />
      <ArchiveSection />
    </div>
  );
